      ...
      <header className={cn(
        ...
        <div className="max-container section-padding">
          <div className="flex items-center justify-between h-[72px]">
            ...
            <nav className="hidden lg:flex items-center gap-0.5">
              {NAV_ITEMS.map((item) => (
                <div key={item.label} className="relative" onMouseEnter={() => item.children && setActiveDropdown(item.label)} onMouseLeave={() => setActiveDropdown(null)}>
                  <Link href={item.href} className={cn('px-3 py-2 text-[12px] font-body font-semibold transition-colors duration-200 flex items-center gap-1 uppercase tracking-[0.15em]', scrolled ? 'text-brand-text/70 hover:text-brand-forest' : 'text-white/80 hover:text-white')}>
            ...
            <Link href="/apply" className="hidden lg:inline-flex items-center justify-center px-6 py-2.5 bg-brand-gold text-white font-body font-semibold text-[11px] tracking-[0.2em] uppercase rounded-sm transition-all duration-300 hover:bg-brand-gold/90 hover:shadow-lg">Application</Link>
