...
import { UserPlus, Mail, Shield, Building2, CheckCircle, Clock, XCircle, Copy, Send } from 'lucide-react';
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setShowInvite(true)} className="btn-portal flex items-center gap-2"><UserPlus className="w-4 h-4" />Invite User</button>
      </div>
          ...
          <div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto p-6">
            ...
            {inviteResult?.url ? (
              <div className="space-y-4">
                ...
                <button onClick={() => { setShowInvite(false); setInviteResult(null); setInviteEmail(''); setInviteName(''); }} className="btn-portal w-full">Done</button>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-xs font-body font-semibold uppercase tracking-wider block mb-1">Full Name</label><input type="text" value={inviteName} onChange={e => setInviteName(e.target.value)} className="input-portal" placeholder="John Doe" /></div>
                  <div><label className="text-xs font-body font-semibold uppercase tracking-wider block mb-1">Email</label><input type="email" value={inviteEmail} onChange={e => setInviteEmail(e.target.value)} className="input-portal" placeholder="user@email.com" /></div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-xs font-body font-semibold uppercase tracking-wider block mb-1">Role</label>
                    <select value={inviteRole} onChange={e => handleRoleChange(e.target.value)} className="input-portal">
                  ...
                  <div><label className="text-xs font-body font-semibold uppercase tracking-wider block mb-1">Portal</label>
                    <select value={invitePortal} onChange={e => setInvitePortal(e.target.value)} className="input-portal">
                      <option value="admin">Admin Portal</option><option value="employer">Employer Portal</option><option value="applicant">Applicant Portal</option>
                    </select></div>
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={handleInvite} disabled={inviteLoading || !inviteEmail || !inviteRole} className="btn-portal flex-1 flex items-center justify-center gap-2 disabled:opacity-50">
      ...
      {/* Users List */}
      <div className="card-portal p-6 mb-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm font-body">
            <thead><tr className="text-left text-xs text-brand-muted uppercase tracking-wider border-b">
              <th className="pb-3 pr-4">User</th><th className="pb-3 pr-4">Role</th><th className="pb-3 pr-4">Portal</th><th className="pb-3 pr-4">Status</th><th className="pb-3">Password</th>
            </tr></thead>
            <tbody>{users.map((u: Record<string, unknown>) => (
              <tr key={u.id as string} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-3 pr-4"><span className="px-2 py-0.5 bg-brand-sage/10 text-brand-sage text-xs rounded font-semibold">{u.role as string}</span></td>
                ...
                <td className="py-3 text-xs">{(u.password_set as boolean) ? <span className="text-green-600">Set</span> : <span className="text-amber-600">Not set</span>}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
