...
import { Settings, Play, CheckCircle, Database, AlertCircle, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm font-body text-brand-muted mt-1">Provision custom fields, verify your GHL setup, and seed test data — all from the browser.</p>
      </div>
      ...
      <div className="card-portal p-6">
        ...
        <p className="text-xs font-body text-brand-muted mb-4">These are NOT saved anywhere — they are only used for this session to call the GHL API directly.</p>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        ...
        >
          ...
          <p className="text-xs text-brand-muted font-body mt-1">Check what already exists in your GHL location — fields, pipelines, contacts, companies.</p>
        </button>
        ...
        >
          ...
          <p className="text-xs text-brand-muted font-body mt-1">Create all custom fields for contacts and companies in your GHL location.</p>
        </button>
        ...
        >
          ...
          <h3 className="font-display font-semibold text-brand-forest text-sm">3. Seed Data</h3>
          <p className="text-xs text-brand-muted font-body mt-1">Create sample contacts and companies for testing.</p>
        </button>
      </div>

      {/* Important note about pipelines */}
      <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 flex gap-3">
        ...
        <div>
          ...
          <p className="text-xs font-body text-amber-700 mt-1">
            GHL does not support creating pipelines via API. After running Provision, go to <strong>GHL → Settings → Pipelines</strong> and create these 5 pipelines:
          </p>
          <ul className="text-xs font-body text-amber-700 mt-2 space-y-1 list-disc list-inside">
            <li><strong>ATS - Lakefront</strong> (11 stages: New Applicant → Closed)</li>
            <li><strong>Business Intake - Lakefront</strong> (10 stages: New Inquiry → Rejected)</li>
            <li><strong>Investor - Lakefront</strong> (7 stages: Inquiry → Declined)</li>
            <li><strong>Provider - Lakefront</strong> (6 stages: Inquiry → Rejected)</li>
            <li><strong>Space Allocation - Lakefront</strong> (6 stages: Inquiry → Closed Lost)</li>
          </ul>
        </div>
      </div>
      ...
      {results?.pipelines && results.pipelines.length > 0 && (
        <div className="card-portal p-6">
          <h3 className="font-display font-semibold text-brand-forest text-sm mb-3">Pipeline IDs (copy to Vercel env vars)</h3>
