...
import { Eye, X, CheckCircle, XCircle, ArrowRight, Briefcase, Building2, Wrench, Warehouse, Users, FileText, Download, Trash2 } from 'lucide-react';

// UPGRADED: Approval dashboard with approve/reject/delete for all entity types
...
export default function ApprovalsPage() {
  ...
  const deleteApp = async (id: string) => { if (!confirm('Delete this application permanently?')) return; await fetch(`/api/applications/${id}`, { method: 'DELETE' }); setDetail(null); loadAll(); };
  const deleteJob = async (id: string) => { if (!confirm('Delete this job permanently?')) return; await fetch(`/api/jobs/${id}`, { method: 'DELETE' }); setDetail(null); loadAll(); };
  const deleteAccount = async (id: string) => { if (!confirm('Delete this user account permanently?')) return; await supabase.from('lf_profiles').delete().eq('id', id); loadAll(); };

  const total = accounts.length + jobs.length + applications.length;
  const tabs = [{ key: 'all' as const, label: 'All Pending', count: total },{ key: 'applications' as const, label: 'Applications', count: applications.length },{ key: 'jobs' as const, label: 'Jobs', count: jobs.length },{ key: 'accounts' as const, label: 'Accounts', count: accounts.length }];
  ...
  return (
    <div className="space-y-6">
      ...
      {showApps && applications.length > 0 && (<div>
        ...
        <div className="bg-white rounded-xl border overflow-hidden"><div className="divide-y divide-gray-50">
          {applications.map(a => { const appType = g(a, 'application_type') || 'employee'; const Icon = TYPE_ICONS[appType] || Users; return (
            <div key={g(a,'id')} className="p-4 flex items-center gap-4 hover:bg-gray-50/50">
              ...
              <div className="flex-1 min-w-0"><div className="flex items-center gap-2 mb-0.5"><p className="text-sm font-body font-semibold text-brand-forest truncate">{g(a,'applicant_name') || 'Unknown'}</p><span className={`px-2 py-0.5 text-[9px] font-semibold rounded-full ${TYPE_COLORS[appType] || 'bg-gray-100 text-gray-500'}`}>{fmt(appType)}</span></div><p className="text-xs text-gray-400 font-body truncate">{g(a,'applicant_email')} &middot; {g(a,'address') || 'No address'}</p></div>
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button onClick={() => deleteApp(g(a,'id'))} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>); })}
        </div></div></div>)}

      {showJobs && jobs.length > 0 && (<div>
        ...
        <div className="bg-white rounded-xl border overflow-hidden"><div className="divide-y divide-gray-50">
          {jobs.map(j => (<div key={g(j,'id')} className="p-4 flex items-center gap-4 hover:bg-gray-50/50">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button onClick={() => deleteJob(g(j,'id'))} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>))}
        </div></div></div>)}

      {showAccounts && accounts.length > 0 && (<div>
        ...
        <div className="bg-white rounded-xl border overflow-hidden"><div className="divide-y divide-gray-50">
          {accounts.map(a => (<div key={g(a,'id')} className="p-4 flex items-center gap-4 hover:bg-gray-50/50">
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              ...
              <button onClick={() => deleteAccount(g(a,'id'))} className="p-1.5 text-gray-300 hover:text-red-500 hover:bg-red-50 rounded-lg" title="Delete"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          </div>))}
        </div></div></div>)}

      {detail && (<div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4"><div className="bg-white rounded-xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-y-auto p-6">
        <div className="flex items-center justify-between mb-4"><h2 className="font-display text-lg font-bold text-brand-forest">{detailType === 'application' ? `Review: ${g(detail, 'applicant_name')}` : `Review: ${g(detail, 'title')}`}</h2><button onClick={() => setDetail(null)}><X className="w-5 h-5 text-gray-400" /></button></div>
        {detailType === 'application' && (<div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm font-body"><div><strong className="text-gray-400 text-xs uppercase">Name</strong><p className="text-brand-forest">{g(detail,'applicant_name')}</p></div><div><strong className="text-gray-400 text-xs uppercase">Email</strong><p>{g(detail,'applicant_email')}</p></div><div><strong className="text-gray-400 text-xs uppercase">Phone</strong><p>{g(detail,'applicant_phone') || '\u2014'}</p></div><div><strong className="text-gray-400 text-xs uppercase">Address</strong><p>{g(detail,'address') || '\u2014'}</p></div></div>
          ...
          <div><label className="text-xs font-semibold uppercase block mb-1 text-gray-400">Move to section after approval</label><select value={moveTarget} onChange={e => setMoveTarget(e.target.value)} className="input-portal text-sm"><option value="">Keep as application only</option><option value="businesses">Move to Businesses</option><option value="services">Move to Services</option></select></div>
        ...
        {detailType === 'job' && (<div className="space-y-3 text-sm font-body">
        ...
        <div className="flex gap-2 mt-6">
          {detailType === 'application' ? (<><button onClick={() => approveApp(g(detail,'id'))} className="flex-1 py-2.5 bg-green-600 text-white rounded-lg text-sm font-semibold">Approve{moveTarget ? ` & Move to ${fmt(moveTarget)}` : ''}</button><button onClick={() => rejectApp(g(detail,'id'))} className="flex-1 py-2.5 bg-red-600 text-white rounded-lg text-sm font-semibold">Reject</button><button onClick={() => deleteApp(g(detail,'id'))} className="py-2.5 px-4 border border-red-200 text-red-500 rounded-lg text-sm font-semibold hover:bg-red-50"><Trash2 className="w-4 h-4" /></button></>) : (<><button onClick={() => approveJob(g(detail,'id'))} className="flex-1 py-2.5 bg-green-600 text-white rounded-lg text-sm font-semibold">Approve & Publish</button><button onClick={() => rejectJob(g(detail,'id'))} className="flex-1 py-2.5 bg-red-600 text-white rounded-lg text-sm font-semibold">Reject</button><button onClick={() => deleteJob(g(detail,'id'))} className="py-2.5 px-4 border border-red-200 text-red-500 rounded-lg text-sm font-semibold hover:bg-red-50"><Trash2 className="w-4 h-4" /></button></>)}
