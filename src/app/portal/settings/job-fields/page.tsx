...
import { Plus, Pencil, Save, X, AlertCircle, RefreshCw } from 'lucide-react';

// ITEM 14: Job Fields Configuration with Kleegr sync button
...
export default function JobFieldsConfigPage() {
  ...
  // ITEM 14: Sync button to push field config to Kleegr
  async function syncToKleegr() {
    ...
    setSyncMsg(`Pushed ${data.pushed || 0} jobs using current field config. ${data.errors?.length ? data.errors.join('; ') : 'No errors.'}`);
    setSyncing(false);
  }

  function addOption(target: 'new' | 'edit') {
    if (!optionInput.value || !optionInput.ghlLabel) return;
    if (target === 'new') setNewField({ ...newField, options: [...newField.options, { ...optionInput }] });
    else setEditForm({ ...editForm, options: [...(editForm.options || []), { ...optionInput }] });
    setOptionInput({ value: '', ghlLabel: '' });
  }
  function removeOption(target: 'new' | 'edit', idx: number) {
    if (target === 'new') setNewField({ ...newField, options: newField.options.filter((_, i) => i !== idx) });
    else setEditForm({ ...editForm, options: (editForm.options || []).filter((_, i) => i !== idx) });
  }
