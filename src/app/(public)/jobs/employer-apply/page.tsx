...
// ITEM 4: Employer application to post jobs and gain access (by invite after approval)
export default function EmployerApplyPage() {
  const [submitted, setSubmitted] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState({ companyName: '', contactName: '', email: '', phone: '', businessType: '', description: '', jobsToPost: '' });

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSubmitting(true);
    // TODO: Save to Supabase + create GHL contact
  ...
  return (
    <div>
        ...
        <div className="max-container section-padding py-20 relative z-10">
          <ScrollReveal>
            <div className="text-center">
              <p className="text-xs tracking-[0.3em] uppercase mb-3 font-body font-semibold" style={{ color: '#C9B97A' }}>For Employers</p>
              <h1 className="font-display text-3xl lg:text-4xl font-bold text-white">Post Jobs at Lakefront</h1>
              <div className="w-12 h-[2px] mx-auto mt-4 mb-4" style={{ backgroundColor: '#C9B97A' }} />
              <p className="text-base text-white/50 font-body max-w-lg mx-auto">Apply for employer access. Once approved, you'll be invited to post jobs and manage applicants.</p>
            </div>
          </ScrollReveal>
        </div>
      </div>
      ...
      <div className="bg-[#FAFAF7] py-16">
        <div className="max-container section-padding max-w-2xl mx-auto">
          {submitted ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-8 text-center">
              <CheckCircle className="w-12 h-12 text-green-500 mx-auto mb-4" />
              <h2 className="font-display text-xl font-semibold text-green-800 mb-2">Application Submitted!</h2>
              <p className="text-sm text-green-600 font-body">We'll review your application and send you an invite once approved. This usually takes 1-2 business days.</p>
            </div>
          ) : (
            <div className="bg-white rounded-xl border border-gray-100 p-8">
              <div className="flex items-center gap-3 mb-6">
                <Building2 className="w-8 h-8 text-brand-forest" />
                <div>
                  <h2 className="font-display text-lg font-semibold" style={{ color: '#2C3E2D' }}>Employer Application</h2>
                  <p className="text-xs font-body text-gray-400">Access is granted by invitation after approval</p>
                </div>
              </div>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div><label className="block text-xs font-body font-medium text-gray-500 mb-1 uppercase tracking-wider">Tell us about your business</label><textarea rows={3} value={form.description} onChange={e => setForm({...form, description: e.target.value})} className="w-full px-4 py-2.5 border border-gray-200 rounded-lg text-sm font-body focus:outline-none focus:border-[#C9B97A] resize-none" /></div>
