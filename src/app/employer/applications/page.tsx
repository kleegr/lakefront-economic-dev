...
export default function EmployerApplicationsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) return;
      const { data: j } = await supabase.from('lf_jobs').select('id, title').eq('created_by', user.id).order('created_at', { ascending: false });
      setJobs(j||[]);
      setLoading(false);
    }
    load();
  ...
  const loadApps = async (jobId: string) => {
    setSelJob(jobId);
    const { data } = await supabase.from('lf_applications').select('*, lf_household_members(full_name, age, years_experience, experience_summary, skills, desired_earnings, availability, qualifications)').eq('job_id', jobId).order('created_at', { ascending: false });
    setApps(data||[]);
  };
  ...
  const viewDetail = (a: R) => {
    setDetail(a);
    setMemberInfo((a.lf_household_members as R)||null);
  };
    ...
    <div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl border p-4">
          <h3 className="text-sm font-body font-semibold text-brand-forest mb-3">Select a Job</h3>
          {jobs.length === 0 ? <p className="text-xs text-gray-400 font-body">No jobs yet.</p> : (
            <div className="space-y-2">{jobs.map(j => (
              <button key={g(j,'id')} onClick={()=>loadApps(g(j,'id'))} className={`w-full text-left p-3 rounded-lg text-sm font-body flex items-center justify-between transition-colors ${selJob===g(j,'id')?'bg-brand-sage/10 text-brand-forest font-semibold':'bg-gray-50 text-gray-600 hover:bg-gray-100'}`}>
                {g(j,'title')}<ChevronRight className="w-3.5 h-3.5" />
              </button>
            ))}</div>
          )}
        </div>
        ...
        <div className="md:col-span-2 bg-white rounded-xl border p-4">
          <h3 className="text-sm font-body font-semibold text-brand-forest mb-3">Applications {selJob && `(${apps.length})`}</h3>
          {!selJob ? <p className="text-xs text-gray-400 font-body text-center py-8">Select a job to see applications.</p> : apps.length === 0 ? <p className="text-xs text-gray-400 font-body text-center py-8">No applications for this job.</p> : (
            <div className="space-y-2">{apps.map(a => {
              ...
              return (
                <div key={g(a,'id')} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg cursor-pointer hover:bg-gray-100" onClick={()=>viewDetail(a)}>
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-brand-sage/10 flex items-center justify-center"><User className="w-4 h-4 text-brand-forest" /></div>
                    <div><div className="text-sm font-body font-semibold text-brand-forest">{g(m,'full_name')||'Applicant'}</div><div className="text-xs text-gray-400">{g(m,'years_experience')} yrs exp &middot; Applied {new Date(g(a,'created_at')).toLocaleDateString()}</div></div>
                  </div>
                  <div className="flex items-center gap-2">
                    <select value={g(a,'status')} onClick={e=>e.stopPropagation()} onChange={e=>{e.stopPropagation(); updateStatus(g(a,'id'),e.target.value);}} className="text-xs border border-gray-200 rounded px-2 py-1">
                      <option value="submitted">Submitted</option><option value="reviewing">Reviewing</option><option value="interview">Shortlisted</option><option value="offered">Offered</option><option value="hired">Hired</option><option value="rejected">Rejected</option>
                    </select>
                  </div>
                </div>
              );
