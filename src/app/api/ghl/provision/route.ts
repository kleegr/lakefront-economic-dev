...
export async function POST(req: NextRequest) {
  ...
  if (action === 'verify') {
    ...
    for (const p of pipeList) log.push(`  - ${p.name} (${p.id}) - ${p.stages?.length || 0} stages`);
  ...
  if (action === 'provision') {
    ...
    for (const field of CONTACT_FIELDS) {
      const fb: any = { name: field.name, dataType: field.dataType, model: 'contact' };
      if (field.options) fb.options = field.options.map(o => ({ value: o }));
      ...
      if (res.ok && res.data?.customField?.id) {
        ...
        log.push(`SKIP ${field.name} - ${res.error?.substring(0, 100) || 'already exists'}`);
      }
    }
    ...
    for (const field of COMPANY_FIELDS) {
      const fb: any = { name: field.name, dataType: field.dataType, model: 'company' };
      if (field.options) fb.options = field.options.map(o => ({ value: o }));
      const res = await ghlApi('POST', `/locations/${locationId}/customFields`, token, fb);
      if (res.ok && res.data?.customField?.id) {
  ...
  if (action === 'seed') {
    ...
    for (const a of applicants) {
      ...
      else log.push(`SKIP ${a.firstName} ${a.lastName} - ${res.error?.substring(0, 100) || 'may exist'}`);
    }
    const companies = [
    ...
    for (const c of companies) {
      ...
      else log.push(`SKIP ${c.name} - ${res.error?.substring(0, 100) || 'may exist'}`);
    }
    log.push('');
