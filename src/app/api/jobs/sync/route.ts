...
export async function POST(req: NextRequest) {
  ...
  if (direction === 'push' || direction === 'both') {
    ...
    for (const job of (jobs || [])) {
      if (!forceAll && job.ghl_record_id && job.ghl_synced_at) { const sa = new Date(job.ghl_synced_at).getTime(); const ua = job.updated_at ? new Date(job.updated_at).getTime() : 0; if (ua <= sa) { results.skipped++; continue; } }
    ...
    for (const sj of (syncedJobs || [])) { if (sj.ghl_record_id && !ghlIds.has(sj.ghl_record_id)) { await supabase.from('lf_jobs').delete().eq('id', sj.id); results.deleted++; } }
  }

  if (direction === 'pull' || direction === 'both') {
    const ghlRecords = await fetchAllGhlJobs();
    for (const record of ghlRecords) { const jobData = ghlPropertiesToJob(record.properties, fields); if (!jobData.title) continue; const { data: existing } = await supabase.from('lf_jobs').select('id').eq('ghl_record_id', record.id).maybeSingle();
      if (existing) { const { error } = await supabase.from('lf_jobs').update({ ...jobData, ghl_record_id: record.id, ghl_synced_at: new Date().toISOString() }).eq('id', existing.id); if (!error) results.pulled++; else results.errors.push(`Pull update: ${error.message}`); }
      else { const slug = String(jobData.title).toLowerCase().replace(/[^a-z0-9]+/g, '-'); const { error } = await supabase.from('lf_jobs').insert({ title: jobData.title, company_name: jobData.company_name || null, location: jobData.location || 'Lakefront Estates, Okeechobee, FL', category: jobData.category || 'General', job_type: jobData.job_type || 'full-time', status: 'draft', visibility: 'public', slug, is_public: true, approval_status: 'approved', ghl_record_id: record.id, ghl_synced_at: new Date().toISOString(), created_by: user.id }); if (!error) results.pulled++; else results.errors.push(`Pull create: ${error.message}`); } }
  }
