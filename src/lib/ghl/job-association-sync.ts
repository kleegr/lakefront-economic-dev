...
//
// Portal → GHL:
//   1. Job created/edited → upsert Job Opening record, associate employer contact
//   2. Employee applies → upsert contact, create ATS opportunity, associate to Job Opening
//   3. Status change (hired/offered/rejected) → update contact + opportunity + tags
//
// GHL → Portal (webhook):
//   4. Job Opening edited in GHL → update lf_jobs
//   5. Opportunity moved in pipeline → update lf_applications.status
//   6. Contact associated to Job Opening → create lf_job_assignments
// ============================================================
...
export async function syncEmployeeToJob(
  ...
  const statusGhl: Record<string, string> = { submitted: 'New', reviewing: 'Reviewing', interview: 'Interview', offered: 'Offered', hired: 'Hired', rejected: 'Rejected', withdrawn: 'Withdrawn' };

  const customFields: Array<{ id: string; field_value: string }> = [];
  const add = (k: string, v: string) => { const e = cf(map, k, v); if (e) customFields.push(e); };
  add('contact.contact_type', 'Employee');
  add('contact.employee_app_status', statusGhl[app.status] || app.status || 'New');
  ...
  if (app.cover_letter) add('contact.cover_letter', app.cover_letter);
...
export async function processGhlWebhook(body: any, supabase: any): Promise<{ action: string; details?: any }> {
  ...
  if (eventType === 'OpportunityStatusUpdate' || eventType === 'OpportunityStageUpdate') {
    ...
    const map: Record<string, string> = { open: 'reviewing', won: 'hired', lost: 'rejected', abandoned: 'withdrawn' };
    const ns = map[oppStatus];
    if (ns && ns !== app.status) { await supabase.from('lf_applications').update({ status: ns, updated_at: new Date().toISOString() }).eq('id', app.id); return { action: 'status_synced', details: { from: app.status, to: ns } }; }
  ...
  if (eventType === 'CustomObjectAssociation') {
    ...
    if (app?.applicant_id) { await supabase.from('lf_job_assignments').upsert({ job_id: job.id, employee_id: app.applicant_id, employer_id: job.employer_id, application_id: app.id, role: 'employee', status: 'active' }, { onConflict: 'job_id,employee_id,role' }); return { action: 'assignment_from_app' }; }
